...
const TRIGGER_TYPES = [
  { value: 'onReady', label: 'On Ready', description: 'When node is first activated' },
  ...
  { value: 'onSignal', label: 'On Signal', description: 'When receiving a custom signal' },
...
export const TriggerSection = ({ triggers, onTriggersChange }: TriggerSectionProps) => {
  const [selectedTriggerType, setSelectedTriggerType] = useState<string>("");

  const handleAddTrigger = () => {
    if (selectedTriggerType) {
      const triggerType = TRIGGER_TYPES.find(t => t.value === selectedTriggerType);
      if (triggerType) {
