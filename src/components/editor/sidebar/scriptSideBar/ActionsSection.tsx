...
const DraggableAction = ({ action, onDelete, onUpdate }: {
  action: Action;
...
export const ActionsSection = ({ actions, onActionsChange }: ActionsSectionProps) => {
  ...
  const handleAddAction = () => {
    if (selectedActionType) {
      ...
      const newAction = ActionFactory.createAction(selectedActionType, actionName);
    ...
    <Card>
      ...
      <CardContent className="space-y-4">
        <p className="text-xs text-muted-foreground">
          Create visual actions using forms - no coding required! Drag them to Main Loop or Triggers.
        </p>
        ...
        <div className="space-y-2">
          ...
          >
            ...
            Add {selectedActionType ? ActionFactory.getActionDisplayName(selectedActionType) : 'Action'}
          </Button>
        </div>
        ...
        <div className="space-y-3">
          {actions.length === 0 ? (
            <div className="text-center py-8 border-2 border-dashed border-gray-200 rounded-lg">
              ...
              <p className="text-xs text-muted-foreground">Select a category and action type above to get started</p>
            </div>
          ) : (
            actions.map((action) => (
              <DraggableAction
        ...
        <div className="border-t pt-3">
          ...
          <div className="flex flex-wrap gap-1">
            {[ActionType.MOVE_BY, ActionType.ROTATE_BY, ActionType.SET_VISIBLE, ActionType.WAIT, ActionType.PRINT].map((actionType) => (
              <Button
                key={actionType}
                size="sm"
                variant="outline"
                onClick={() => {
                  const actionName = ActionFactory.getActionDisplayName(actionType);
                  const newAction = ActionFactory.createAction(actionType, actionName);
                  onActionsChange([...actions, newAction]);
                }}
