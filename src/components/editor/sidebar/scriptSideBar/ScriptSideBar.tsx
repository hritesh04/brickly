  ...
  const handlePreviewCode = () => {
    ...
    import("./utils/CodeGenerator").then(({ CodeGenerator }) => {
      const mainLoopCode = CodeGenerator.generateMainLoopFunction(
        mainLoopActions.map(dropped => actions.find(a => a.id === dropped.actionId)).filter(Boolean) as Action[]
      );
      
      const triggerCodes = triggers.map(trigger => 
        CodeGenerator.generateTriggerFunction(
          trigger.name,
          trigger.actions.map(dropped => actions.find(a => a.id === dropped.actionId)).filter(Boolean) as Action[]
        )
      );

      const fullCode = [mainLoopCode, ...triggerCodes].filter(Boolean).join('\n\n');
    ...
    if (activeData?.type === 'action' && overData?.type === 'main-loop') {
      ...
      setTriggers(triggers.map(trigger => {
        if (trigger.id === triggerId) {
          return {
            ...trigger,
            actions: [...trigger.actions, { ...newDroppedAction, order: trigger.actions.length }]
          };
        }
        return trigger;
      }));
        ...
        <div className="p-4 space-y-4">
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-muted-foreground">Node: {activeNode.name}</p>
            </div>
      ...
      >
        {draggedAction ? (
          <div className="bg-white border rounded-lg p-2 shadow-lg max-w-[200px] pointer-events-none">
            <span className="text-sm font-medium truncate block">{draggedAction.name}</span>
          </div>
        ) : null}
      </DragOverlay>
    </DndContext>
