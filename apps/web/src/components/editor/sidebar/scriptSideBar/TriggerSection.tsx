...
import {
  getAvailableTriggersForNode,
  getTriggerOptionsForNode,
} from "@brickly/types";
...
const TriggerItem = ({
  ...
  const handleRemoveAction = (actionId: string) => {
    const updatedActions = trigger.actions.filter(
  ...
  const triggerOptions = getTriggerOptionsForNode(nodeType as any);
...
export const TriggerSection = ({
  ...
  const handleAddTrigger = () => {
    if (selectedTriggerType) {
      const triggerType = triggerOptions.find(
        (t) => t.value === selectedTriggerType
      );
      if (triggerType) {
  ...
  const handleTriggerActionsChange = (
    triggerId: string,
    actions: DroppedAction[]
  ) => {
    onTriggersChange(
